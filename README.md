...
This Nix package directly fetches and patches the active upstream [Autodesk-Fusion-360-on-Linux](https://codeberg.org/Lolig4/Autodesk-Fusion-360-on-Linux) repository (maintained by Lolig4 and cryinkfly) via `fetchFromGitea`. During build time, Nix patches out distro package manager checks, `sudo` calls, and interactive prompts, wiring up isolated Wine Wow64 compatibility and dependencies from the Nix store.
...
- **Graphics Pipeline**:
  ...
  - **Viewport Refresh Forcer**: Automatically deploys the `ViewportRefreshForcer` Python add-in to prevent the 3D viewport canvas from turning black or freezing when panning/orbiting under Wine.
...
fusion360 install        # Run the upstream installer directly
fusion360 status         # Show active prefix, Wine version, and installation status
fusion360 fix-navbar     # Run upstream navigation bar flicker fix
...
| `FUSION360_REFRESH_FORCER` | Set to `0` to disable the `ViewportRefreshForcer` add-in | `1` |
...
- **Browser Login / SSO**: If the browser does not redirect back into Fusion after logging in, verify that the protocol handler is registered:
  ...
  ```
- **Black Viewport Canvas After Panning**: When using DXVK under Wine, offscreen surface blits can occur before the present completes when ending a pan/orbit motion. `fusion360` automatically bundles and enables the `ViewportRefreshForcer` add-in to continually refresh the active viewport, resolving the issue. If you still encounter viewport glitches:
  - Focus the Fusion window and press `Ctrl+Alt+N` to toggle the navigation toolbar.
  - Or switch to OpenGL mode with `FUSION360_DXVK=0 fusion360`.
